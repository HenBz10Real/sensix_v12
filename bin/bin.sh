...
cmd="cmd notification post -S bigtext -t \"SENSIX WEB-UI\" \"Tag\" \"Status: Actived | Author: Henpeex\""
eval "$cmd"
...
while true; do
    ...
    if [ -n "$window_buffer" ]; then
        if [ "$prev_window_state" != "active" ]; then
            ...
            sleep 0.5
            wm density "$calibrar"
            cmd="cmd notification post -S bigtext -t \"SENSIX WEB-UI\" \"Tag\" \"Process injecting something\""
            eval "$cmd"
            sleep 2
            ...
            cmd="cmd notification post -S bigtext -t \"SENSIX WEB-UI\" \"Tag\" \"Successfully configuration dpi\""
            eval "$cmd"
        fi
